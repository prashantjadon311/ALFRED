...
export class WorkflowRunsRepository extends BaseRepository<WorkflowRunDoc> {
  ...
  async incrementUsage(id: ObjectId, userId: ObjectId, inputTokens: number, outputTokens: number, costUsd: number, workspaceId?: ObjectId) {
    const filter = workspaceId ? { _id: id, userId, workspaceId } : { _id: id, userId };
    await this.collection().updateOne(filter as any, { $inc: { totalInputTokens: inputTokens, totalOutputTokens: outputTokens, totalCostUsd: costUsd, version: 1 } as any, $set: { updatedAt: new Date() } });
  }
}
